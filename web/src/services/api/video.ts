...
type VideoResponse = { id?: string; request_id?: string; task_id?: string; status?: string; error?: { message?: string }; url?: string; result_url?: string; video_url?: string; video?: { url?: string; respect_moderation?: boolean } | null; content?: { video_url?: string; url?: string } | null };
...
async function createOpenAIVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: VideoMediaOptions): Promise<VideoGenerationTask> {
    const grokVideo = /^grok-imagine-video(?:-|$)/i.test(modelOptionName(model));
    if (grokVideo) {
        const { path, body } = await buildGrokVideoRequest(config, model, prompt, references, options);
        try {
            ...
            const id = created.request_id || created.id || created.task_id;
...
async function buildGrokVideoRequest(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: VideoMediaOptions) {
    const modelName = modelOptionName(model);
    const isVideo15 = modelName === "grok-imagine-video-1.5" || modelName === "grok-imagine-video-1.5-preview";
    const mode = config.videoMode || "frames";
    const videoInputs = options?.videos || [];
    const audioInputs = options?.audios || [];
    const voiceIds = (config.videoVoiceIds || "").split(",").map((value) => value.trim()).filter(Boolean);
    const keyframeTimes = (config.videoKeyframeTimes || "").split(/[,\s]+/).filter(Boolean).map(Number);
    if (mode === "edit" || mode === "extend") {
        if (videoInputs.length !== 1 || references.length || audioInputs.length || voiceIds.length || keyframeTimes.length) {
            throw new Error("Grok 视频编辑或续接需要恰好一个视频输入，不能同时连接图片或音频参考");
        }
        const source = videoInputs[0];
        const videoUrl = !source.storageKey && /^https:\/\//i.test(source.url) ? source.url : await readFileAsDataUrl(await referenceMediaToFile(source, "ref.mp4", "invalidReferenceVideo", options));
        const duration = mode === "extend" ? grokVideoDuration(config.videoSeconds) : undefined;
        if (duration !== undefined && (duration < 2 || duration > 10)) throw new Error("Grok 续接时长需为 2–10 秒");
        const body = { model: modelName, prompt, video: { url: videoUrl }, ...(duration !== undefined ? { duration } : {}) };
        return { path: mode === "edit" ? "/videos/edits" : "/videos/extensions", body };
    }
    if (videoInputs.length) throw new Error("请在视频设置中选择“编辑”或“续接”模式使用视频参考");
    if (!prompt.trim() && !references.length && !voiceIds.length && !audioInputs.length) throw new Error("Grok 文生视频需要提示词或参考素材");
    if (voiceIds.length > 3 || audioInputs.length + voiceIds.length > 3) throw new Error("Grok 最多支持 3 个音频参考");
    if ((voiceIds.length || audioInputs.length || keyframeTimes.length || config.videoPinLast === "true") && !isVideo15) {
        throw new Error("预置声音、关键帧和固定尾帧需要 grok-imagine-video-1.5");
    }
    const duration = grokVideoDuration(config.videoSeconds);
    const resolution = normalizeVideoResolution(config.vquality);
    const aspectRatio = videoAspectRatio(config.size);
    if (!["1:1", "16:9", "9:16", "4:3", "3:4", "3:2", "2:3"].includes(aspectRatio)) throw new Error(`Grok 不支持 ${aspectRatio} 视频比例`);
    const images = await Promise.all(references.map((image) => imageToDataUrl(image)));
    if (images.some((url) => !url)) throw new Error(apiText("referenceImageReadFailed"));
    if (config.videoPinLast === "true" && images.length === 0) throw new Error("固定尾帧需要连接一张图片");
    const remaining = [...images];
    const first = (mode === "frames" || config.videoPinFirst === "true") && remaining.length ? remaining.shift() : undefined;
    const last = ((mode === "frames" && remaining.length) || config.videoPinLast === "true") && remaining.length ? remaining.pop() : undefined;
    if (config.videoPinFirst === "true" && !first) throw new Error("固定首帧需要连接一张图片");
    if (config.videoPinLast === "true" && !last) throw new Error("固定尾帧需要额外连接一张图片");
    if (last && !isVideo15) throw new Error("固定尾帧需要 grok-imagine-video-1.5；经典版可改用“参考图”模式");
    if (keyframeTimes.length > 4 || keyframeTimes.some((time) => !Number.isFinite(time) || time <= 0 || time >= duration || Math.abs(time * 3 - Math.round(time * 3)) > 0.01) || new Set(keyframeTimes.map((time) => Math.round(time * 3))).size !== keyframeTimes.length) {
        throw new Error("关键帧最多 4 个，时间需在视频内部且相隔至少 1/3 秒");
    }
    if (keyframeTimes.length > remaining.length) throw new Error("关键帧时间数量超过可用参考图数量");
    const keyframes = keyframeTimes.map((time) => ({ image: { url: remaining.shift()! }, timestamp_s: time }));
    if (remaining.length > 7) throw new Error("Grok 每次最多支持 7 张参考图");
    if (!isVideo15 && first && remaining.length) throw new Error("经典版 Grok 不支持首帧与参考图组合；请改用 1.5 模型");
    if ((remaining.length || keyframes.length || last || voiceIds.length || audioInputs.length) && resolution === "1080p") {
        throw new Error("Grok 参考图/音频及首尾帧组合最高支持 720p");
    }
    if (!isVideo15 && resolution === "1080p") throw new Error("1080p 需要 grok-imagine-video-1.5");
    const audioFiles = await Promise.all(audioInputs.map((audio) => referenceMediaToFile(audio, "ref.mp3", "invalidReferenceAudio", options)));
    const referenceAudios = [...voiceIds.map((voice_id) => ({ voice_id })), ...await Promise.all(audioFiles.map(async (file) => ({ url: await readFileAsDataUrl(file) })))];
    const body = {
        model: modelName,
        prompt,
        duration,
        ...
        ...(remaining.length ? { reference_images: remaining.map((url) => ({ url })) } : {}),
...
function grokVideoDuration(value: string) {
    ...
    if (!Number.isInteger(duration) || duration < 1 || duration > 15) throw new Error("Grok 视频时长需为 1–15 秒");
    return duration;
}
...
async function pollOpenAIVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        ...
        if (video.video?.respect_moderation === false) return { status: "failed", error: "视频未通过上游内容审核" };
        if (video.status === "failed" || video.status === "cancelled" || video.status === "expired") return { status: "failed", error: readApiErrorMessage(video.error?.message) || apiText("videoGenerationFailed") };
...
function videoResultUrl(payload: VideoResponse) {
    return [payload.video?.url, payload.video_url, payload.result_url, payload.url, payload.content?.video_url, payload.content?.url].find((url) => typeof url === "string" && (isPublicMediaUrl(url) || /\.mp4(\?|#|$)/i.test(url)));
}
