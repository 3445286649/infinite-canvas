...
type GenerationLogConfig = Pick<AiConfig, "model" | "videoModel" | "size" | "vquality" | "videoSeconds" | "videoGenerateAudio" | "videoWatermark" | "videoMode" | "videoPinFirst" | "videoPinLast" | "videoKeyframeTimes" | "videoVoiceIds">;
...
export default function VideoPage() {
    ...
    const previewGenerationLog = (log: GenerationLog) => {
        ...
        updateConfig("videoPinFirst", log.config.videoPinFirst);
        updateConfig("videoPinLast", log.config.videoPinLast);
        updateConfig("videoKeyframeTimes", log.config.videoKeyframeTimes);
        updateConfig("videoVoiceIds", log.config.videoVoiceIds);
...
function normalizeLogConfig(log: Partial<GenerationLog>): GenerationLogConfig {
    return {
        ...
        videoPinFirst: log.config?.videoPinFirst || "false",
        videoPinLast: log.config?.videoPinLast || "false",
        videoKeyframeTimes: log.config?.videoKeyframeTimes || "",
        videoVoiceIds: log.config?.videoVoiceIds || "",
    };
}
...
function buildLog({ prompt, model, config, references, durationMs, status, task, video, error }: { prompt: string; model: string; config: AiConfig; references: ReferenceImage[]; durationMs: number; status: GenerationLog["status"]; task?: VideoGenerationTask; video?: GeneratedVideo; error?: string }): GenerationLog {
    const logConfig = {
        ...
        videoPinFirst: config.videoPinFirst,
        videoPinLast: config.videoPinLast,
        videoKeyframeTimes: config.videoKeyframeTimes,
        videoVoiceIds: config.videoVoiceIds,
    };
    return {
        id: nanoid(),
        createdAt: Date.now(),
...
function buildVideoConfig(config: AiConfig, model: string): AiConfig {
    return {
        ...
        videoSeconds: /^grok-imagine-video(?:-|$)/i.test(modelOptionName(model)) ? config.videoSeconds : normalizeVideoSeconds(config.videoSeconds),
        vquality: normalizeResolution(config.vquality),
