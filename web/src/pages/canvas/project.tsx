...
function InfiniteCanvasPage() {
                    ...
                    prev.map((item) =>
                        item.id === node.id
                            ? applyGeneratedVideo(item, video, {
                                  ...
                                  videoPinFirst: generationConfig.videoPinFirst,
                                  videoPinLast: generationConfig.videoPinLast,
                                  videoKeyframeTimes: generationConfig.videoKeyframeTimes,
                                  videoVoiceIds: generationConfig.videoVoiceIds,
                              })
                            : item,
                    ),
                );
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData, imageId?: string) => {
            ...
            if (!prompt && !(node.type === CanvasNodeType.Video && /^grok-imagine-video(?:-|$)/i.test(modelOptionName(generationConfig.model)) && (retryImages.length || context?.referenceVideos.length || context?.referenceAudios.length || generationConfig.videoVoiceIds))) {
